...
export default function ApplicationPanel() {
  ...
  const [statusFilter, setStatusFilter] = useState<"all" | "shortlisted" | "pending">("all")
  ...
  const toggleBookmark = async (id: string) => {
    ...
    try {
      ...
      if (!res.ok) {
        const errorData = await res.json()
        throw new Error(errorData.message || "Bookmark шинэчлэхэд алдаа гарлаа")
      }

      const data = await res.json()
      setApplications((prev) => prev.map((app) => (app._id === id ? data.data : app)))

      toast.success(data.data.bookmarked ? "Bookmark хийгдлээ" : "Bookmark цуцлагдлаа")
    } catch (error) {
      console.error("Bookmark алдаа:", error)
      toast.error(error instanceof Error ? error.message : "Bookmark солиход алдаа гарлаа")
    } finally {
      setUpdatingId(null)
    }
  }
  ...
  const filteredApplications = applications
    .filter((app) => {
      ...
      if (selectedJobId !== "all" && (!app.jobId || app.jobId._id !== selectedJobId)) return false
      return true
    })
    .sort((a, b) => {
  ...
  const handleSubmitJob = async (e: React.FormEvent) => {
    ...
    const filteredRequirements = jobForm.requirements.filter((req) => req.trim() !== "")

    if (!jobForm.title || !jobForm.description || filteredRequirements.length === 0) {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(editingJobId ? "Failed to update job" : "Failed to create job")
      }

      toast.success(editingJobId ? "Ажлын байр амжилттай шинэчлэгдлээ" : "Ажлын байр амжилттай үүслээ")

      setJobForm({ title: "", description: "", requirements: [""] })
      setEditingJobId(null)
      fetchJobs()
    } catch {
      toast.error(editingJobId ? "Ажлын байр шинэчлэхэд алдаа гарлаа" : "Ажлын байр үүсгэхэд алдаа гарлаа")
    } finally {
      setIsSubmitting(false)
    }
  }
          ...
          {selectedCvUrl && (
            <div className="-mt-[30px]">
              {selectedCvUrl.endsWith(".pdf") ? (
                <embed src={selectedCvUrl} type="application/pdf" width="100%" height="600px" className="" />
              ) : (
                <Image
                  width={1000}
                  height={600}
                        ...
                        <Card className="border-0 shadow-lg bg-white p-0 hover:shadow-xl transition-shadow duration-300">
                          <CardContent className="p-6">
                            <div className="flex items-center gap-2 mb-4">
                              ...
                              <h3 className="font-semibold text-lg">Ажлын байрны мэдээлэл</h3>
                            </div>
                            ...
                            <div className="space-y-4">
                              ...
                              <div>
                                <h5 className="font-medium text-gray-900 mb-3">Шаардлагууд:</h5>
                              ...
                              <div className="pt-4 border-t border-gray-100">
                                <div className="flex items-center justify-between text-sm">
                                  ...
                                  <span className="font-medium text-blue-600">{filteredApplications.length} CV</span>
                                </div>
                              </div>
                            </div>
                          </CardContent>
                    ...
                    <Card className="border-0 shadow-lg bg-white/80 backdrop-blur-sm hover:shadow-xl transition-shadow duration-300">
                      <CardContent className="p-6">
                        <div className="flex flex-col space-y-4">
                          {/* Job Filter */}
                          <div>
                            <Label className="text-sm font-medium text-gray-700 mb-2 block">Ажлын байраар шүүх</Label>
                          ...
                          <Tabs
                            value={statusFilter}
                            onValueChange={(v) => setStatusFilter(v as "all" | "shortlisted" | "pending")}
                          >
                                ...
                                <div className="flex-1">
                                  ...
                                  <div className="mb-3">
                                    <Badge variant="outline" className="text-xs">
                                      ...
                                      {application.jobId?.company && ` - ${application.jobId.company}`}
                                    </Badge>
                                  </div>
                                  ...
                                  <div className="mb-4">
                                    <div className="flex gap-2">
                                      ...
                                      <p className="text-gray-800 font-medium mb-2">{application.aiSummary.lastName}</p>
                                    </div>
                                ...
                                <div className="flex flex-col items-end gap-3 ml-6">
                                  ...
                                  <div className="flex items-center gap-2">
                                    <button
                                      ...
                                      className={`p-2 rounded-md transition-colors ${application.bookmarked
                ...
                <Card className="border-0 shadow-lg bg-white/80 backdrop-blur-sm hover:shadow-xl transition-shadow duration-300">
                  <CardContent className="p-8">
                    ...
                    {jobs.length > 0 ? (
                          ...
                          >
                            <div className="flex justify-between items-start">
                              <div className="flex-1">
                                ...
                                <div className="mt-3">
                                  <p className="text-sm font-medium text-gray-700 mb-2">Шаардлагууд:</p>
                                ...
                                <p className="text-xs text-gray-500 mt-3">Үүсгэсэн: {formatDate(job.createdAt)}</p>
                              </div>
                              <div className="flex gap-2">
                                <Button
                                  size="sm"
                      ...
                      <p className="text-center text-gray-500 py-8">Одоогоор ажлын байр байхгүй байна</p>
                    )}
                  </CardContent>
                </Card>
              </TabsContent>
